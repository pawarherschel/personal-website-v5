...
<div class="hidden lg:flex transition-all items-center h-11 mr-2 rounded-lg
      bg-black/[0.04] hover:bg-black/[0.06] focus-within:bg-black/[0.06]
      dark:bg-white/5 dark:hover:bg-white/10 dark:focus-within:bg-white/10
" id="search-bar">
    <Icon class="absolute text-[1.25rem] pointer-events-none ml-3 transition my-auto text-black/30 dark:text-white/30" icon="material-symbols:search"></Icon>
    <input bind:value={keywordDesktop} class="transition-all pl-10 text-sm bg-transparent outline-0
         h-full w-40 active:w-60 focus:w-60 text-black/50 dark:text-white/50" on:focus={() => search(keywordDesktop, true)}
           placeholder="{i18n(I18nKey.search)}"
    >
</div>

<!-- toggle btn for phone/tablet view -->
<button aria-label="Search Panel" class="btn-plain scale-animation lg:!hidden rounded-lg w-11 h-11 active:scale-90" id="search-switch"
        on:click={togglePanel}>
    <Icon class="text-[1.25rem]" icon="material-symbols:search"></Icon>
</button>

<!-- search panel -->
<div class="float-panel float-panel-closed search-panel absolute md:w-[30rem]
top-20 left-4 md:left-[unset] right-4 shadow-2xl rounded-2xl p-2" id="search-panel">
  ...
  " id="search-bar-inside">
        <Icon class="absolute text-[1.25rem] pointer-events-none ml-3 transition my-auto text-black/30 dark:text-white/30" icon="material-symbols:search"></Icon>
        <input bind:value={keywordMobile} class="pl-10 absolute inset-0 text-sm bg-transparent outline-0
